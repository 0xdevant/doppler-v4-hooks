...
import { State } from "doppler/src/initializers/Doppler.sol";
import { MigrationMath } from "doppler/src/libraries/MigrationMath.sol";
import { DopplerLensReturnData, DopplerLensTest } from "doppler/test/unit/DopplerLens.t.sol";
...
contract DopplerV4PocTest is DopplerLensTest {
    uint256 public constant SALE_DURATION = 6 hours;
    ...
    function test_buy_InitHighDemandThenFixedBuyUntilMaxProceed() public {
        ...
        uint256 BUY_NUMERAIRE_AMOUNT = 0.5 ether;
        ...
        while (totalEthProceeds < DEFAULT_MAXIMUM_PROCEEDS) {
            require(
                ...
                string.concat(
                    ...
                    vm.toString(totalEpochs),
                    ", please use a bigger BUY_NUMERAIRE_AMOUNT"
                )
            );
            ...
            try lensQuoter.quoteDopplerLensData(
                IV4Quoter.QuoteExactSingleParams({
                    poolKey: key,
                    zeroForOne: !isToken0,
                    exactAmount: count <= HIGH_DEMAND_EPOCHS
                        ? uint128(HIGH_DEMAND_BUY_AMOUNT)
                        : uint128(BUY_NUMERAIRE_AMOUNT),
                    hookData: ""
                })
            ) {
                (tokenBought,) = count <= HIGH_DEMAND_EPOCHS
                    ? buy(-int256(HIGH_DEMAND_BUY_AMOUNT))
                    : buy(-int256(BUY_NUMERAIRE_AMOUNT));
            } catch (bytes memory) {
                console.log("\n");
            ...
            try lensQuoter.quoteDopplerLensData(
                ...
                DopplerLensReturnData memory lensData = lensQuoter.quoteDopplerLensData(
                    IV4Quoter.QuoteExactSingleParams({
                        poolKey: key, zeroForOne: !isToken0, exactAmount: 1, hookData: ""
                    })
                );
    ...
    function test_buy_FixedBuyAtEachEpochUntilMaxProceed() public {
        ...
        while (totalEthProceeds < DEFAULT_MAXIMUM_PROCEEDS) {
            ...
            try lensQuoter.quoteDopplerLensData(
                IV4Quoter.QuoteExactSingleParams({
                    poolKey: key, zeroForOne: !isToken0, exactAmount: uint128(BUY_NUMERAIRE_AMOUNT), hookData: ""
                })
            ) {
                (tokenBought,) = buy(-int256(BUY_NUMERAIRE_AMOUNT));
            } catch (bytes memory) {
                console.log("\n");
                console.log("REVERTED, stopped the simulation.");
                break;
    ...
    function test_buy_FixedBuyWithinFixedEpochsUntilMinOrMaxProceed() public {
        ...
        uint256 BUY_NUMERAIRE_AMOUNT = isMaxProceed
        ...
        for (uint256 i; i < FIXED_EPOCHS; i++) {
            ...
            //         poolKey: key,
            //         zeroForOne: !isToken0,
            //         exactAmount: uint128(BUY_NUMERAIRE_AMOUNT),
            //         hookData: ""
            //     })
            // ) {
            //     (tokenBought,) = buy(-int256(BUY_NUMERAIRE_AMOUNT));
    ...
    function test_buy_EmptyFixedEpochs_BuyWithinFixedEpochsUntilMinOrMaxProceed() public {
        ...
        // uint256 BUY_NUMERAIRE_AMOUNT = 0.2 ether;
